...
if(round.lat.lon) lat.lon.dp <- 4 #' If rounded, choose number of decimal places

#' Select particular taxa. To include all taxa set `species.selection` to NULL
if(!exists('species.selection')) species.selection <- NULL

save.compiled.data <- TRUE

displayPlots <- TRUE # repress plotting for speedier run-time by setting FALSE
savePlots <- FALSE # as above, setting FALSE represses plot saving which speeds things up
...
if(save.compiled.data){
  ...
  f <- paste0(paste('compiled data', paste(s, collapse = '_'), sep = '_'), '.csv')
