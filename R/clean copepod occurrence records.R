...
#' Choose some latitudinal limits to filter data
lat_lim <- c(-90,-30) #' selected for Southern Ocean data
lat_lim <- sort(lat_lim)
hemisphere <- ifelse(all(lat_lim <= 0), 'south', 'north')
lat_bound <- switch(hemisphere,
                    south = lat_lim[2],
                    north = lat_lim[1])
...
# dir.temp <- file.path(getwd(), 'temp')
# dir.map <- file.path(dir.data.base, 'map files', 'Natural Earth')
...
invisible(sapply(paste(dir.functions, R_functions, sep = '/'), source, local = .GlobalEnv))
rm(R_functions)
...
#' `Move this option into the comoilation script`

#' convert to spatial object
# dat <- st_as_sf(
#   dat, coords = c('decimalLongitude', 'decimalLatitude'), remove = FALSE)
# dat <- switch(hemisphere,
#               south = dat[dat$decimalLatitude <= lat_bound,],
#               north = dat[dat$decimalLatitude >= lat_bound,])
# dat <- setCRS(dat)

# # Plot filtered data onto map, colouring by species
# plt_cols <- colourScale_paired(length(speciesSelected), speciesSelected)
# 
# # Subset data needed for plot
# dat_plt <- unique(dat[c('decimalLongitude','decimalLatitude','species')]) # unique coords and species
# dat_plt$species <- factor(dat_plt$species, levels = speciesSelected, labels = speciesSelected)
# 
# # Plotting function
# mapRecordsBySpecies <- function(x, Data, base_map, Title = NULL, Subtitle = NULL){
#   plt <- base_map + 
#     new_scale_colour() + 
#     geom_sf(data = Data, aes(colour = species), size = 0.5) + 
#     guides(colour = guide_legend(ncol = 4, byrow = TRUE, override.aes = list(size = 4))) + 
#     scale_color_manual(name = element_blank(), values = plt_cols)
#   if(!is.null(Title)) plt <- plt + labs(title = paste(x, Title))
...
# }
# 
# MainTitle <- bquote(The ~ most ~ sampled ~ species ~ comprising ~ .(100*propRecords2Keep) * '%' ~ of ~ all ~ occurrence ~ records ~ south ~ of ~ .(abs(lat_bound)) * degree * S)
# Title <- 'copepod occurrence records'
# 
# # Create plots
# plt_maps_2 <- mapRecordsBySpecies(Source, dat_plt, mapData$map_plot, Title)
# leg <- get_legend(plt_maps_2)
# plt_maps_2 <- plt_maps_2 + theme(legend.position = 'none')
# DrawMainTitle <- ggdraw() + draw_label(MainTitle, fontface = 'bold', fontfamily = 'serif')
# 
# plt_maps_2 <- plot_grid(
#   DrawMainTitle, plt_maps_2,
#   ncol = 1, rel_heights = c(0.05,0.95))
# 
# plt_maps_2.leg <- plot_grid(
#   DrawMainTitle, plt_maps_2, leg,
#   ncol = 1, rel_heights = c(0.05,0.55,0.4))
# if(displayPlots) print(plt_maps_2)
# 
# rm('dat_plt')
# 
...
#' #' Some plots
#' yr <- range(dat$year, na.rm = TRUE)
#' yrs <- yr[1]:yr[2]
#' dy <- setNames(sapply(yrs, function(z) sum(dat$year == z, na.rm = TRUE)), yrs)
#' if(displayPlots)
#'   barplot(dy)
#' 
#' dm <- outer(dat$month, month.abb, '==')
#' dm <- unlist(apply(dm, 1, which))
#' db <- setNames(rep(0, 12), month.abb)
#' for(i in 1:12) db[i] <- sum(dm == i)
#' if(displayPlots)
#'   barplot(db)
