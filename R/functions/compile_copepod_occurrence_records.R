...
compile.data <- function(species.selection = NULL,
  ...
  dat$Species[i] <- 'Acartia (acanthacartia) fossae'
  i <- dat$Species == 'Acartia (acanthacartia) sinjiensis'
  dat$Species[i] <- 'Acartia (Acanthacartia) sinjiensis'
  i <- dat$Species == 'Acartia (acanthacartia) steueri'
  dat$Species[i] <- 'Acartia (Acanthacartia) steueri'
  i <- dat$Species == 'Acartia (acanthacartia) tonsa'
  dat$Species[i] <- 'Acartia (Acanthacartia) tonsa'
  i <- dat$Species == 'Acartia (acartia) danae'
  dat$Species[i] <- 'Acartia (Acartia) danae'
  i <- dat$Species == 'Acartia (acartiura) clausi'
  dat$Species[i] <- 'Acartia (Acartiura) clausi'
  i <- dat$Species == 'Acartia (acartiura) discaudata'
  dat$Species[i] <- 'Acartia (Acartiura) discaudata'
  i <- dat$Species == 'Acartia (acartiura) ensifera'
  dat$Species[i] <- 'Acartia (Acartiura) ensifera'
  i <- dat$Species == 'Acartia (acartiura) jilletti'
  dat$Species[i] <- 'Acartia (Acartiura) jilletti'
  i <- dat$Species == 'Acartia (acartiura) longiremis'
  dat$Species[i] <- 'Acartia (Acartiura) longiremis'
  i <- dat$Species == 'Acartia (acartiura) simplex'
  dat$Species[i] <- 'Acartia (Acartiura) simplex'
  i <- dat$Species == 'Acartia (acartiura) teclae'
  dat$Species[i] <- 'Acartia (Acartiura) teclae'
  i <- dat$Species == 'Acartia (acartiura) tranteri'
  dat$Species[i] <- 'Acartia (Acartiura) tranteri'
  i <- dat$Species == 'Acartia (odontacartia) pacifica'
  dat$Species[i] <- 'Acartia (Odontacartia) pacifica'
  i <- dat$Species == '  Acartia (odontacartia) spinicauda'
  dat$Species[i] <- '  Acartia (Odontacartia) spinicauda'
  i <- dat$Species == 'Acartia ensifera'
  dat$Species[i] <- 'Acartia (Acartiura) ensifera'
  i <- dat$Species == 'Acartia fossae'
  dat$Species[i] <- 'Acartia (Acanthacartia) fossae'
  i <- dat$Species == 'Acartia jilletti'
  dat$Species[i] <- 'Acartia (Acartiura) jilletti'
  i <- dat$Species == 'Acartia longiremis'
  dat$Species[i] <- 'Acartia (Acartiura) longiremis'
  i <- dat$Species == 'Acartia longiremis'
  dat$Species[i] <- 'Acartia (Acartiura) longiremis'
  i <- dat$Species == 'Acartia pacifica'
  dat$Species[i] <- 'Acartia (Odontacartia) pacifica'
  i <- dat$Species == 'Acartia simplex'
  dat$Species[i] <- 'Acartia (Acartiura) simplex'
  i <- dat$Species == 'Acartia sinjiensis'
  dat$Species[i] <- 'Acartia (Acanthacartia) sinjiensis'
  i <- dat$Species == 'Acartia teclae'
  dat$Species[i] <- 'Acartia (Acartiura) teclae'
  i <- dat$Species == 'Acartia tonsa'
  dat$Species[i] <- 'Acartia (Acanthacartia) tonsa'
  i <- dat$Species == 'Acartia tranteri'
  dat$Species[i] <- 'Acartia (Acartiura) tranteri'
  ...
  i <- dat$Species == 'Calanus helgolandicus'
  dat$Species[i] <- 'Calanus helgolandicus helgolandicus'
  ...
  i <- dat$Species != 'Copilia hendorffi'
  dat <- lapply(dat, function(z) z[i])
  ...
  i <- dat$Species == 'Metridia longa'
  dat$Species[i] <- 'Metridia longa longa'
  ...
  i <- dat$Species == 'Oithona setigera'
  dat$Species[i] <- 'Oithona setigera setigera'
  i <- dat$Species != 'Oncaea bathyalis'
  dat <- lapply(dat, function(z) z[i])
  i <- dat$Species != 'Oncaea neobscura'
  dat <- lapply(dat, function(z) z[i])
  i <- dat$Species != 'Oncaea obscura'
  dat <- lapply(dat, function(z) z[i])
  i <- dat$Species != 'Oncaea obtusa'
  dat <- lapply(dat, function(z) z[i])
  ...
  i <- dat$Species == 'Pleuromamma quadrungulata'
  dat$Species[i] <- 'Pleuromamma quadrungulata quadrungulata'
  ...
  i <- dat$Species == 'Tisbe furcata'
  dat$Species[i] <- 'Tisbe furcata furcata'
  i <- dat$Species == 'Tortanus (tortanus) barbatus'
  dat$Species[i] <- 'Tortanus (Tortanus) barbatus'
  i <- dat$Species == 'Tortanus (tortanus) gracilis'
  dat$Species[i] <- 'Tortanus (Tortanus) gracilis'
  i <- dat$Species == 'Tortanus barbatus'
  dat$Species[i] <- 'Tortanus (Tortanus) barbatus'
  i <- dat$Species == 'Tortanus gracilis'
  dat$Species[i] <- 'Tortanus (Tortanus) gracilis'
