...
## Code details
#### Scripts
Code scripts are contained in the `R` directory. The entire procedure for cleaning and compiling the data, and producing some output plots & summary statistics, is implemented through `run_data_prep.R`, which is the only script that needs to be opened. This simple script sources three functions, contained in the `R/functions` directory, that are run in succession to clean the original data, compile it into a single table, then produce plots.
...
+ Cairo 1.6.2, cowplot 1.1.3, data.table 1.17.0, dplyr 1.1.4, ggh4x 0.3.1, ggplot2 3.5.2, ggpmisc 0.6.2, ggnewscale 0.5.1, ggpp 0.5.9, ggrepel 0.9.6, ggtext 0.1.2, ggthemes 5.1.0, patchwork 1.3.0, R.utils 2.13.0, RColorBrewer 1.1.3, reshape2 1.4.4, sf 1.0.20, sp 2.2.0.
